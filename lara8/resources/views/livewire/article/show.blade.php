    ...
    <div>
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between">
                <button wire:click="create()" class="border-2 border-indigo-400 bg-indigo-500 text-white font-bold py-2 px-4 rounded my-3">New Article</button>
                @if($isDialogOpen)
                    @include('livewire.article.create')                    
                @endif      
                <a href="{{ route('categories') }}" class="border-2 border-indigo-400 text-indigo-600 font-bold py-2 px-4 rounded my-3"> Categories </a>
            </div>
                    ...
                    @forelse($articles as $item)
                        <tr class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            ...
                            <td class="px-4 py-2">{{ substr($item->body, 0, 100) }} 
                                <span class="text-xs px-2 font-bold bg-gray-400 text-white rounded py-0.5"> <a href="articles/{{ $item->id }}">...more</a> </span>
                            </td>
